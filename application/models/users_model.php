...
class Users_Model extends CI_Model {
    ...
    function take_button_credits($post) {
         $DB_MAIN = $this->load->database('default', TRUE);
        $query = $DB_MAIN->select('credits')->where('id', $post['id'])->get('store_users');
        if ($query->num_rows() > 0) {
            $query = $query->row()->credits;
            settype($query, "integer");
            $data = $query + $post['action'];
            $data = array(
                'credits' => $data,
            );
            $DB_MAIN->where('id', $post['id']);
    ...
    function add_button_credits($post) {
        $DB_MAIN = $this->load->database('default', TRUE);
        $query = $DB_MAIN->select('credits')->where('id', $post['id'])->get('store_users');
        if ($query->num_rows() > 0) {
            $query = $query->row()->credits;
    ...
    function add_item($post) {
        ...
        $data = array(
            ...
            'item_id' => $post['item'],
            'acquire_date' => date("Y-m-d H:i:s"),
            'aquire_method' => 'web',
        );
        $DB_MAIN->insert('store_users_items', $data);
    }

    function add_button_item($post) {
         $DB_MAIN = $this->load->database('default', TRUE);
        $data = array(
            'user_id' => $post['user_id'],
            'item_id' => $post['specitemadd'],
            'acquire_date' => date("Y-m-d H:i:s"),
            'aquire_method' => 'web',
        );
        $DB_MAIN->insert('store_users_items', $data);       
